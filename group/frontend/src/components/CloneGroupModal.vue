  ...
  <BaseModal size="large" @close="$emit('close')">
    <div class="tx-form-page">
      ...
      <template v-if="!loading && !fatalError">
        <div class="tx-header">
          <h2 class="tx-title">Clone group <strong class="tx-title-highlight-name">{{ groupName }}</strong></h2>
        </div>
...
import { fetchCloneData, submitForm } from '../api.js'
...
onMounted(async () => {
  try {
    const payload = await fetchCloneData(props.groupName)
    ...
    errorMsg.value = err.message || 'Failed to load clone data.'
    fatalError.value = true
  } finally {
    loading.value = false
  }
...
async function onSubmit() {
  ...
  errorMsg.value = ''
  try {
    const { ok, data } = await submitForm(`/clone/${encodeURIComponent(props.groupName)}`, fd)
    if (ok || data.status === 'success') {
      emit('saved')
      emit('close')
    } else {
      errorMsg.value = data.message || 'Clone failed.'
    }
  } catch (err) {
