...
class Helper():
    ...
    def filter_data(self, table=None, data=None):
        ...
        for ele in data:
            if 'name' in data[ele]:
                name = data[ele]["name"]
            if 'osimage' in data[ele]:
                osimage = data[ele]["osimage"]
            if name:
                action = self.action_items(table, osimage, name)
                for row in rows:
                    if name in row:
    ...
    def action_items(self, table=None, osimage=None,name=None):
        """
        ...
        if item_type == 'button':
            button = "btn btn-sm "
            info = f'<a href="/show/{osimage}" class="{button}btn-info">Info</a>'
            delete = f'<a href="/delete/{osimage}/{name}" class="{button}btn-danger">Delete</a>'
        elif item_type == 'icon':
            info =  self.make_icon(
                href=url_for('show', osimage=osimage),
                onclick=None,
            ...
            delete =  self.make_icon(
                href=url_for('delete', osimage=osimage, tag=name),
