  ...
  let warningToast = $(
    '<div class="bs-toast toast toast-placement-ex m-2 fade bg-warning top-0 end-0 show" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="2000"> \
        <div class="toast-header"> \
          <i class="bx bx-bell me-2"></i> \
          <div class="me-auto fw-medium">Configuration</div> \
          <small>0 seconds ago</small> \
  ...
  function getLatestValues(count) {
    var nhc
    if ($('#rule_nhc_' + count).is(':checked') === true) {
      nhc = 'yes'
    } else {
      nhc = 'no'
    }
  ...
  function setupEventListeners(count) {
    $('#rule_name_' + count).on('input', function () {
      getLatestValues(count)
    })
    $('#rule_description_' + count).on('input', function () {
      getLatestValues(count)
    })
    $('#rule_for_' + count).on('input', function () {
      getLatestValues(count)
    })
    $('#exprInput_' + count).on('input', function () {
      getLatestValues(count)
    })
    $('#rule_status_' + count).on('change', function () {
      getLatestValues(count)
    })
    $('#rule_nhc_' + count).on('change', function () {
      getLatestValues(count)
    })
  }
      ...
      success: function (data) {
        ...
        data.groups.forEach(function (group) {
          group.rules.forEach(function (rule) {
            ...
            row +=
              '<td><a href="" data-bs-toggle="modal" data-bs-target="#rule_modal_' +
              count +
              '">' +
              rule.alert +
              '</a></td>'
            row +=
              '<td><div class="form-check form-switch mb-2"><input class="form-check-input" onclick="update_configuration(`status`, this,' +
              count +
              ', `' +
              btoa(JSON.stringify(rule)) +
              '`);" type="checkbox" id="rule_status" ' +
              (rule.labels._trix_status !== false ? 'checked' : '') +
              '><label class="form-check-label" for="rule_status" id="rule_status_text_' +
              count +
              '">' +
              (rule.labels._trix_status !== false ? 'ON' : 'OFF') +
              '</label></div></td>'
            row +=
              '<td><div class="form-check form-switch mb-2"><input class="form-check-input" onclick="update_configuration(`nhc`, this,' +
              count +
              ', `' +
              btoa(JSON.stringify(rule)) +
              '`);" type="checkbox" id="rule_nhc" ' +
              (rule.labels.nhc === 'yes' ? 'checked' : '') +
              '><label class="form-check-label" for="rule_nhc" id="rule_nhc_text_' +
              count +
              '">' +
              (rule.labels.nhc === 'yes' ? 'ON' : 'OFF') +
              '</label></div></td>'
            row +=
              '<td><select id="severity_' +
              count +
              '" onchange="update_configuration(`severity`, this, ' +
              count +
              ', `' +
              btoa(JSON.stringify(rule)) +
              '`);" class="form-select form-select-sm ' +
              (rule.labels.severity === 'critical' ? 'btn-dark' : 'btn-' + rule.labels.severity) +
              '"><option class="btn-primary" value="">Set Priority</option> \
                                        <option class="btn-dark" value="critical" ' +
              (rule.labels.severity === 'critical' ? 'selected' : '') +
              '>Critical</option><option class="btn-danger" value="danger" ' +
              (rule.labels.severity === 'danger' ? 'selected' : '') +
              '>Danger</option> \
                                        <option class="btn-warning" value="warning" ' +
              (rule.labels.severity === 'warning' ? 'selected' : '') +
              '>Warning</option><option class="btn-info" value="info" ' +
              (rule.labels.severity === 'info' ? 'selected' : '') +
              '>Informational</option> \
                                        </select></td>'
            row +=
              '<td> \
                                    <button class="tooltip-modal-link" data-bs-toggle="modal" data-bs-target="#rule_modal_' +
              count +
              '" id="actions" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" data-bs-original-title="<i class=\'bx bxs-arrow-from-left bx-xs\'></i> <span>Edit This Rule</span>"><i class="bx bx-md bx-edit" style="color: #696cff;"></i></button> \
                                    <button class="tooltip-modal-link" onclick="update_configuration(`delete`, this,' +
              count +
              ', `' +
              btoa(JSON.stringify(rule)) +
              '`);" id="actions" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" data-bs-original-title="<i class=\'bx bxs-arrow-from-left bx-xs\'></i> <span>Delete This Rule</span>"><i class="bx bx-md bx-trash" style="color: red;"></i></button> \
                                  </td>'
            tableBody.append(row)

            var modal =
              '<div class="modal fade" id="rule_modal_' +
              count +
              '" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-xl" role="document"><div class="modal-content"> \
                                    <div class="modal-header"><h5 class="modal-title" id="exampleModalLabel4">RULE: ' +
              rule.alert +
              '</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div> \
                                    <div class="modal-body"> \
                                        <div class="row"><div class="col mb-12"> \
                                            <button type="button" id="button_html_' +
              count +
              '" onclick="rule_modal_html(' +
              count +
              ', `1`);" class="btn btn-secondary btn-sm">Switch to HTML Mode</button> \
                                            <button type="button" id="button_json_' +
              count +
              '" onclick="rule_modal_json(' +
              count +
              ', `2`);" class="btn btn-primary btn-sm">Switch to JSON Mode</button> \
                                            <button type="button" id="button_yaml_' +
              count +
              '" onclick="rule_modal_yaml(' +
              count +
              ', `3`);" class="btn btn-warning btn-sm">Switch to YAML Mode</button> \
                                            <div id="ruleEditor_' +
              count +
              '" style="display: none; height: 300px; border: 1px solid #ddd;"></div></div></div> \
                                        <div id="model-form_' +
              count +
              '"> \
                                            <div class="row g-6"> \
                                                <div class="col mb-0"><label for="rule_name_' +
              count +
              '" class="form-label">Rule Name</label><input type="text" id="rule_name_' +
              count +
              ...
              '"></div> \
                                            ...
                                            <div class="row"> \
                                                <div class="col mb-6"><label for="rule_description_' +
              count +
              '" class="form-label">Rule Description</label><input type="text" id="rule_description_' +
              count +
              '" class="form-control" placeholder="Enter Name" value="' +
              ...
              '"></div> \
                                            ...
                                            <div class="row"> \
                                                <div class="col mb-0"><label for="rule_for_' +
              count +
              '" class="form-label">Rule For</label><input type="text" id="rule_for_' +
              count +
              '" class="form-control" placeholder="Enter For" value="' +
              rule.for +
              '"></div> \
              ...
              count +
              '" class="form-label">Enable</label><div class="form-check form-switch "><input type="checkbox" onclick="update_configuration(`status`, this,' +
              count +
              ', `' +
              btoa(JSON.stringify(rule)) +
              '`);" id="rule_status_' +
              ...
              (rule.labels._trix_status !== false ? 'ON' : 'OFF') +
              '</label></div></div> \
                                                <div class="col mb-6"><label for="rule_nhc_' +
              count +
              '" class="form-label">Rule NHC</label><div class="form-check form-switch "><input type="checkbox" onclick="update_configuration(`nhc`, this,' +
              count +
              ', `' +
              btoa(JSON.stringify(rule)) +
              '`);" id="rule_nhc_' +
              ...
              (rule.labels.nhc === 'yes' ? 'ON' : 'OFF') +
              '</label></div></div> \
                                                <div class="col mb-6"><label for="rule_severity_' +
              count +
              '" class="form-label">Set Priority</label> \
                                                    <select id="rule_severity_' +
              count +
              '" onchange="update_configuration(`severity`, this, ' +
              count +
              ', `' +
              ...
              '>Informational</option> \
                                    ...
                                    <div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button><button type="button" onclick="update_configuration(`save`, this,' +
              count +
              ', `' +
              btoa(JSON.stringify(rule)) +
              '`);" class="btn btn-primary">Save Rule</button></div></div></div></div>'
        ...
        var add_modal =
          '<div class="modal fade" id="rule_modal_" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-xl" role="document"><div class="modal-content"> \
          ...
          '" class="form-control promql" placeholder="Enter Name" @keyup="handlePromql" ></div> \
                                ...
                                <div class="row"> \
                                    <div class="col mb-0"><label for="rule_status_' +
          add_count +
          '" class="form-label">Enable</label><div class="form-check form-switch "><input type="checkbox" onchange="document.getElementById(`rule_status_label_' +
          add_count +
          "`).textContent = this.checked ? 'ON' : 'OFF';\"  id=\"rule_status_" +
          add_count +
          '" class="form-check-input"><label id="rule_status_label_' +
          add_count +
          '" for="rule_status_' +
          add_count +
          '" class="form-check-label">OFF</label></div></div> \
                                    <div class="col mb-6"><label for="rule_nhc_' +
          add_count +
          '" class="form-label">Rule NHC</label><div class="form-check form-switch "><input type="checkbox" onchange="document.getElementById(`rule_nhc_label_' +
          add_count +
          "`).textContent = this.checked ? 'ON' : 'OFF';\" id=\"rule_nhc_" +
          add_count +
          '" class="form-check-input"><label id="rule_nhc_label_' +
          add_count +
          '" for="rule_nhc_' +
          add_count +
          '" class="form-check-label">OFF</label></div></div> \
          ...
          '" class="form-label">Set Priority</label> \
                                        <select id="rule_severity_' +
          add_count +
          '" onchange="this.classList.remove(`btn-primary`, `btn-dark`, `btn-danger`, `btn-warning`, `btn-info`); const selectedClass = classMap[this.value]; if (selectedClass) { this.classList.add(selectedClass); }" class="form-select form-select-sm"><option class="btn-primary">Set Priority</option> \
                        ...
                        <div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button><button type="button" onclick="add_rule(`save`, this,' +
          add_count +
          ', `save`);" class="btn btn-primary">Save Rule</button></div></div></div></div>'
      ...
      configuration.groups.forEach(function (group) {
        ...
        if (content) {
          ...
          if (valid_alert_name) {
            ...
            if (
              !document.getElementById('rule_severity_' + count).value ||
              document.getElementById('rule_severity_' + count).value.trim() === ''
            ) {
            ...
            displayToast(
              failedToast,
              'A valid alert name is required, Expecting at least one alphabet or number',
            )
          }
        }
      })
        ...
        if (count_configuration === count) {
          if (key === 'status') {
            if (element.checked === rule.labels._trix_status) {
              console.log(
                'No Need to update, both values are equal ' +
                  key +
                  ' >> ' +
                  element.checked +
                  ' >> ' +
                  rule.labels._trix_status +
                  ' ',
              )
            } else {
              ...
              if ($(element).attr('id') === 'rule_status') {
                update = true
              }
            }
          }
          if (key === 'nhc') {
            if (
              (element.checked === true && rule.labels.nhc === 'yes') ||
              (element.checked === false && rule.labels.nhc === 'no')
            ) {
              console.log(
                'No Need to update, both values are equal ' +
                  key +
              ...
              if ($(element).attr('id') === 'rule_nhc') {
                update = true
              }
            }
          }
          if (key === 'severity') {
            document.getElementById('severity_' + count).className = document
              .getElementById('severity_' + count)
              .className.replace(/\bbtn-\S+/g, '')
              .trim()
            document.getElementById('rule_severity_' + count).className = document
              .getElementById('severity_' + count)
              .className.replace(/\bbtn-\S+/g, '')
              .trim()
            ...
            if (!element.value || element.value.trim() === '') {
              ...
              console.log(
                'No Need to update, ' + key + " >>  can't be empty, You have to select a level.",
              )
            } else {
              rule.labels.severity = element.value
              document.getElementById('severity_' + count).classList.add('btn-' + element.value)
              document
                .getElementById('rule_severity_' + count)
                .classList.add('btn-' + element.value)
              if ($(element).attr('id') === 'severity_' + count) {
                update = true
              }
            }
          }
